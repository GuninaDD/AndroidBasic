...
public class WthrFragment extends Fragment {
    ...
    private static final String APP_PREFERENCES_PRESSURE = "Pressure";
    private static final String APP_PREFERENCES_WIND = "Wind";
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        initViews(view);
        updateWeatherData(getContext(), getParcel().getCityName());

    }

    private void updateWeatherData(final Context context, String cityName) {
    ...
    private void saveLastCity(String cityName) {
        ...
        editor.putBoolean(APP_PREFERENCES_WIND, setWind);
        editor.putBoolean(APP_PREFERENCES_PRESSURE, setPressure);
        editor.apply();
    }
    ...
    private void loadWthr(RenderWthr render) {
        if (Double.parseDouble(render.getCurrentWthrWind().substring(0, 2)) >= 10) {
            showNotification();
        }
    ...
    private void showNotification() {
        String CHANNEL_ID = "Wind channel";
        NotificationCompat.Builder builder =
                new NotificationCompat.Builder(Objects.requireNonNull(getContext()), CHANNEL_ID)
                        .setSmallIcon(R.drawable.ic_launcher_background)
                        .setContentTitle("Предупреждение")
                        .setContentText("Внимание, в Вашем регионе наблюдаются сильные порывы ветра, будте осторожнее")
